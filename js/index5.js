...
(function () {
  ...
  function slider(rootEl, slideSel, arrowSel, nowSel, dwell) {
    ...
    /* a film on the opening slide starts with the page */
    film(slides[current], true);
  ...
  /* --- The standard box ------------------------------------------------------
         ...
         phone the boxes are in flow and keep their own height. A box marked
         `data-own-height` (direction 5's lease plate on the beach) keeps its
         own height, so it never reaches up onto the car. */
  var boxes = Array.prototype.slice.call(document.querySelectorAll(".box:not([data-own-height])"));
  ...
  function readStops() {
    var vh = window.innerHeight;
    var list = [];
    /* the chapters are sticky: a chapter the page has scrolled past reports
       the stuck position, not its own, and read then every chapter above the
       screen would sit at the current scroll — no stop left to step up to.
       So the stick is lifted for the measure (synchronously, nothing paints
       in between) and each chapter's own place in the flow is read. */
  ...
  function step(direction) {
    ...
    readStops();   /* fresh each step: late pictures and the rail change the flow */
